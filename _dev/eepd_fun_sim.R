...
.eepd_sim <- function(model,
    ...
    if(log == FALSE){
        ...
        if(length(formula.tools::get.vars(model)) > 1) { 
            new_form_names = c(paste(formula.tools::get.vars(model)[1], "diff", diff_k, sep = "_"),
                               formula.tools::get.vars(model)[-1])
        }
        else {
        ...
        if(length(new_form_names) > 1) {
            new_form = as.formula(paste(new_form_names[1], "~", paste(new_form_names[-1], collapse = "+")))
        }
        else{
        ...
        if(is.null(family)){
            ...
            treated_val_preds_trans_scale = sapply(X = 1:nrow(treated_train_fits),
                                                   FUN = function(x) { 
                                                       as.numeric(treated_val_mod_mats[["mod_mat"]][[x]] %*% mvrnorm(
                                                           n = 1,
                                                           mu = coef(treated_train_fits[["mod"]][[x]]),
                                                           Sigma = vcov(treated_train_fits[["mod"]][[x]]))) 
                                                   })
            control_val_preds_trans_scale = sapply(X = 1:nrow(control_train_fits), 
                                                   FUN = function(x) { 
                                                       as.numeric(control_val_mod_mats[["mod_mat"]][[x]] %*% mvrnorm(n = 1,
                                                                                                                     mu = coef(control_train_fits[["mod"]][[x]]),
                                                                                                                     Sigma = vcov(control_train_fits[["mod"]][[x]]))) })
            ...
            treated_val_preds_trans_scale = sapply(X = 1:nrow(treated_train_fits),
                                                   FUN = function(x) { 
                                                       as.numeric(treated_val_mod_mats[["mod_mat"]][[x]] %*% mvrnorm(n = 1,
                                                                                                                     mu = coef(treated_train_fits[["mod"]][[x]]),
                                                                                                                     Sigma = vcov(treated_train_fits[["mod"]][[x]]))) })
            control_val_preds_trans_scale = sapply(X = 1:nrow(control_train_fits), 
                                                   FUN = function(x) { 
